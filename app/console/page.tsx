  ...
  useEffect(() => {
    ...
    function initHoloMap() {
        ...
        Object.values(allModuleBlueprints).forEach(module => {
            if (Array.isArray(module.interconexoes)) { // CORREÇÃO APLICADA
    ...
    function initToneJs() {
        if (window.Tone && !synth) {
          ...
          // It might not be loaded yet, try again after a short delay
          setTimeout(() => {
            if (window.Tone && !synth) {
               synth = new window.Tone.Synth().toDestination();
            } else if (!window.Tone) {
               console.warn("Tone.js não carregado. Funções de áudio desabilitadas.");
            }
          }, 500);
        }
    }
    ...
    function playTone(frequency) {
        if (!synth) initToneJs(); 
        if (synth) {
          // Wrap in a Tone.start() to handle audio context suspension
