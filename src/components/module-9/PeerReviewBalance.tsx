  ...
  return (
    <div className="p-6 bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 rounded-xl text-white shadow-xl flex flex-col gap-4">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        {votes.map((r) => (
          <div key={r.id} className={`p-4 rounded-lg flex-1 flex flex-col justify-between ${r.color}`} >
            <div>
              <div className="font-semibold">{r.id}</div>
              <div className="text-sm opacity-80 mt-1">Voto: <span className="font-bold capitalize">{r.vote}</span></div>
            </div>
            <div className="flex gap-2 mt-4">
              <Button
                onClick={() => castVote(r.id, 'aprova')}
                className="flex-1 bg-green-600 hover:bg-green-500 text-xs md:text-sm"
              >
                Aprovar
              </Button>
              <Button
                onClick={() => castVote(r.id, 'rejeita')}
                className="flex-1 bg-red-600 hover:bg-red-500 text-xs md:text-sm"
              >
                Rejeitar
              </Button>
            </div>
      ...
      <div className="mt-6">
        <h3 className="text-xl font-semibold">Registro Cerimonial</h3>
        <ul className="mt-2 list-disc list-inside text-sm text-gray-300">
          {votes.map((r, idx) => (
            <li key={idx}>
              {r.id}: <span className="font-semibold">{r.vote}</span>
            </li>
          ))}
          {verdict && <li className="text-yellow-400 font-bold mt-2">{verdict}</li>}
        </ul>
      </div>
    </div>
  );
