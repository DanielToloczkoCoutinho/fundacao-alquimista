...
const createLogEntry = (source: 'M25' | 'M1' | 'M2' | 'M7' | 'M8' | 'M98', step: string, message: string, data?: any): AnyLogEntry => ({
    step: `[${source}] ${step}`,
    message,
...
const Modulo1_SegurancaUniversal = (log: LogCallback) => ({
    ReceberAlertaDeViolacao: (alerta: any) => log(createLogEntry('M1', 'ALERTA', `Projeção da Consciência: ${alerta.mensagem}`)),
...
const Modulo8_PIRC = (log: LogCallback) => ({
    IniciarProtocoloCura: (alvo_id: string, tipo_cura: string) => log(createLogEntry('M8', 'Cura', `Iniciando cura '${tipo_cura}' para '${alvo_id}'`)),
});
...
class ModuloAlquimiaConsciencia {
    ...
    constructor(private logCallback: LogCallback) {
        this.logCallback(createLogEntry('M25', 'Inicialização', 'Módulo 25 - Alquimia da Consciência Expandida inicializado.'));
    ...
    private _calcular_coerencia_interna_projetor(frequencias: number[], pureza: number): number {
        this.logCallback(createLogEntry('M25', 'Cálculo', 'Calculando Coerência Interna do Projetor...'));
        if (!frequencias || frequencias.length === 0) return 0.0;
        const soma = frequencias.reduce((a, b) => a + b, 0);
        const eci = (soma * pureza) / PHI;
        this.logCallback(createLogEntry('M25', 'Resultado', `ECI: ${eci.toFixed(2)}`));
        return eci;
    }
    
    private _equacao_estabilidade_psiquica(estresse: number, coerencia_emocional: number): number {
        this.logCallback(createLogEntry('M25', 'Cálculo', 'Calculando Estabilidade Psíquica...'));
        const estabilidade = CONST_TF / (estresse + (1 - coerencia_emocional) + 1e-9);
        this.logCallback(createLogEntry('M25', 'Resultado', `Estabilidade: ${estabilidade.toFixed(2)}`));
        return estabilidade;
    }

    private _equacao_probabilidade_colapso(eci: number, estabilidade: number): number {
        this.logCallback(createLogEntry('M25', 'Cálculo', 'Calculando Probabilidade de Colapso...'));
        const prob = 1 / (eci * estabilidade + 1e-9);
        this.logCallback(createLogEntry('M25', 'Resultado', `Prob. Colapso: ${prob.toFixed(4)}`));
        return prob;
    }
    
    private _equacao_intervencao_alquimica(risco: number, potencial: number): number {
        this.logCallback(createLogEntry('M25', 'Cálculo', 'Calculando Eficácia da Intervenção...'));
        const eficacia = (potencial / (risco + 1e-9)) * CONST_TF;
        this.logCallback(createLogEntry('M25', 'Resultado', `Eficácia: ${eficacia.toFixed(2)}`));
        return eficacia;
    }
    ...
    async avaliar_preparacao_projetor(projetor_id: string, dados_psiquicos: any, intencao: any): Promise<any> {
        this.logCallback(createLogEntry('M25', 'Avaliação', `Avaliando preparação de '${projetor_id}'`));
        ...
        this.logCallback(createLogEntry('M25', 'Conclusão Avaliação', `Status: ${status}`));
        return { status: "SUCESSO", detalhes: registro };
    }

    async monitorar_projecao(projetor_id: string, duracao: number) {
        this.logCallback(createLogEntry('M25', 'Monitoramento', `Monitorando projeção de '${projetor_id}' por ${duracao}s.`));
        for (let i = 0; i < duracao; i++) {
            ...
            this.logCallback(createLogEntry('M25', `Iteração ${i + 1}`, `ECI: ${eci.toFixed(1)}, Estab: ${estabilidade.toFixed(1)}, Risco: ${probColapso.toFixed(4)}`));
            
            if (probColapso > 0.05) {
                this.logCallback(createLogEntry('M25', 'INTERVENÇÃO', `Risco elevado detectado. Iniciando estabilização.`));
...
export const runModuleTwentyFiveSequence = async (logCallback: LogCallback) => {
    ...
    if (avaliacao.status === "SUCESSO") {
        ...
        logCallback(createLogEntry('M25', 'Fim', 'Projeção concluída com sucesso.'));
    } else {
        logCallback(createLogEntry('M25', 'Fim', `Projeção abortada: ${avaliacao.mensagem}`));
    }
};
