...
const np = {
    ...
    log2: Math.log2,
    exp: Math.exp,
    corrcoef: (x: number[], y: number[]): [number[], number[]] => {
        // Simplified Pearson correlation
        if (x.length !== y.length || x.length === 0) return [[1, 0], [0, 1]];
        const n = x.length;
        ...
        if (denominator === 0) return [[1, 0], [0, 1]];
        const r = numerator / denominator;
        return [[1, r], [r, 1]];
    },
    polyfit: (x: number[], y: number[], deg: number): number[] => {
        // Simplified linear regression (deg=1)
        if (deg !== 1 || x.length !== y.length) return [0];
        const n = x.length;
        const sum_x = x.reduce((a, b) => a + b, 0);
        const sum_y = y.reduce((a, b) => a + b, 0);
        const sum_xy = x.reduce((sum, val, i) => sum + val * y[i], 0);
...
biblioteca.registrar({
    ...
    funcao: (p: number[], q: number[]) => {
        ...
        const js_div = 0.5 * p.reduce((sum, pi, i) => sum + (pi * Math.log2(pi / m[i]) + q[i] * Math.log2(q[i] / m[i])), 0);
